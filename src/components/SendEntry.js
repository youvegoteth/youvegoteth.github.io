...
class SendEntry extends Component {
  ...
  componentDidMount() {
    ...
    // new account json stuff of account
    // owner is the address
    this.setState({owner: _owner, private_key: _private_key})
    // document.location.href = 'send.html?address=' + _owner + '&hash=' + _private_key;
  }

  activateLasers = () => {
    this.setState({show: "process"})
  }

  deactivateLasers = () => {
    this.setState({show: "entry"})
  }

  getChainData = () => {
    this.setState({txHash: "pseudo for KOs data", privKey: "pseudo for KOs data", num_eth: "pseudo for KOs data"});
  }
      ...
      return (
        <div className="container">
          <SendProcess
            ...
            private_key={this.state.privKey}
            num_eth={this.state.num_eth}
